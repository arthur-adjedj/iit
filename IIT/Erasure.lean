...
match e with
| const n _ => contains (collectHeaderNames its) n
| app f _  => isHeaderApp f
| _           => false
...
match e with
| forallE n t b _ => mkForall n e.binderInfo (eraseCtor t) (eraseCtor b)
| lam n t b _     => mkLambda n e.binderInfo (eraseCtor t) (eraseCtor b)
| app f e       => if isHeaderApp its f then eraseCtor f -- erase applications of sorts
                     else mkApp (eraseCtor f) (eraseCtor e)
| const n l     => mkConst (addEIfHeader its n) l
| _               => e
