...
Lean.MVarId.withContext mVar do
  Lean.MVarId.checkNotAssigned mVar `inversion
  let target ← Lean.MVarId.getType mVar
  -- Get Prop sorted fields
  let truesgs ← Lean.MVarId.cases (← mkFreshExprMVar $ mkConst `True).mvarId! fVar
  ...
  let fields ← Lean.MVarId.withContext trueMVar do
  ...
  for (e : Expr) in fields do
    let (names',fieldFVar, mVar') ← Lean.MVarId.withContext mVar do
      let fieldMVar ← mkFreshExprMVar e
      let fsgs ← Lean.MVarId.cases fieldMVar.mvarId! fVar
      Lean.MVarId.assumption fsgs[0]!.mvarId
      ...
      Lean.MVarId.assign mVar $ mkApp fMVar fieldMVar
      let (fieldFVar, mVar') ← Lean.MVarId.intro fMVar.mvarId! name
