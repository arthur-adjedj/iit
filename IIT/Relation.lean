...
match t with
| app f e   => let fm := appFn! tm -- TODO i suspect this needs tweeking for ext indices
                 let em := appArg! tm
...
| sort _       => mkForall "x" BinderInfo.default ref (mkSort l)
| _ => e
...
| forallE n t b _ =>
  match headerAppIdx? its t with
  | some _ => let ref := mkApp (liftBVarsTwo ref) $ mkBVar 1
              let t' := liftBVarsOne t
...
match e with
  | sort _ => let dref := mkApp (liftBVarsOne dref) (mkBVar 0)
...
| const n l => let t := addRIfHeader its n l
...
| forallE n t b _ =>
  match headerAppIdx? its t with
  | some _ => let td ← methodTmS its methods motives (liftBVarsOne t) t
              let td := mkApp td (mkBVar 0)
